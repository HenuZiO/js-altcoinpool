function loadNetworkStats(poolData, poolBlocksData) {
  ...
  let networkHashrate = formatHash(poolData.pool.networkStats.networkHashrate, 5, 'H/s');
  let networkDiff = formatHash(poolData.pool.networkStats.networkDifficulty, 5, '');
  let lastChainBlock = new Date(poolData.pool.networkStats.lastNetworkBlockTime).toLocaleString();

  let blockReward = poolBlocksData[1] ? poolBlocksData[1].reward.toFixed(2) : 'No info';
        ...
        <div class="main-stats__info">
          <ul class="main-stats__list">
            ...
            <li class="main-stats__list-item">
              ...
              <span class="main-stats__span-right">${
                formattedNetworkBlockTime ? formattedNetworkBlockTime : 'No info'
              }</span>
            </li>
          </ul>
        </div>
