function loadAdditionalStats(poolData, poolBlocksData, minerData) {
  ...
  let mostRecentBlockTimeInSeconds = new Date(mostRecentBlockTime).getTime() / 1000;
  let mostAncientBlockTimeInSeconds = new Date(mostAncientBlockTime).getTime() / 1000;
  ...
  let blocksTTF = minerData.performance ? (networkHashrate / minerHash) * networkBlockTime : '';
  ...
  const htmlPendingShares = $('.additional-stats__item-text--pending').text(minerPendingShares);
  ...
  `);
  const html24hEstEarnings = $('.additional-stats__item-text--esteanings').html(`
    ${
  ...
  `);
  const htmlLastPayment = $('.additional-stats__item-text--payment').text(minerLastPayment);
  const htmlLastPaymentLink = minerLastPaymentLink
    ? $('.additional-stats__item-link--payment').attr('href', `${minerLastPaymentLink}`)
    : $('.additional-stats__item-link--payment').text('No payments yet');
  const htmlNetworkBlockTime = $('.additional-stats__item-text--blocktime').text(
