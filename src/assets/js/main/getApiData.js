...
async function getPoolData() {
  try {
    const response = await fetch(`https://api.altcoinpool.ru/api/pools/${currentPool}`);
    if (!response.ok) throw new Error(`HTTP error: ${response.status}`);
    const data = await response.json();
    return data;
  } catch (error) {
...
async function getMinersData() {
  try {
    const response = await fetch(`https://api.altcoinpool.ru/api/pools/${currentPool}/miners/`);
    if (!response.ok) throw new Error(`HTTP error: ${response.status}`);
    const data = await response.json();
    return data;
  } catch (error) {
